...
void inode_table_destroy() {
    for (int i = 0; i < INODE_TABLE_SIZE; i++) {
        if (inode_table[i].nodeType != T_NONE) {
          ...
          pthread_rwlock_destroy(&inode_table[i].lock); /* checkar*/
    }
}
...
/*verificar erros unlock*/
void deslockando_state(int inumber){
...
/* verificar_erros_escrita*/
void lockando_operations_write(int inumber){
    if ((pthread_rwlock_wrlock(&inode_table[inumber].lock))!=0){
    printf("Erro a dar lock write\n" );
    exit(EXIT_FAILURE);
...
/*verificar_erros_leitura*/
void lockando_operations_read(int inumber){
    if((pthread_rwlock_rdlock(&inode_table[inumber].lock))!=0){
    printf("Erro a dar lock read\n" );
    exit(EXIT_FAILURE);
